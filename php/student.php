<?php
    ...
    do {
      do {
        $sqlSelectDeanLister = "SELECT db_studNo FROM ems_dean_lister WHERE db_year ='$ARRAYYEAR[$X]' AND db_section ='$ARRAYSECTION[$Y]' ORDER BY db_gwa ASC LIMIT 5";
        ...
        $Y++;
      } while ($Y<sizeof($ARRAYSECTION));
      $Y=0;
      $X++;
    } while ($X<sizeof($ARRAYYEAR));
      ...
      <tbody>
        <?php
          if ($data) {
            do {
        ?>
            <tr id='<?php echo $data['db_studNo']?>' <?php if (in_array($data['db_studNo'], $arrayDeanList)) {?>style='background-color:cyan'<?php }?>>
              <td><?php echo $data['db_studNo'] ?></td>
              <td style='text-align: left;'><?php echo $data['db_Sname'].", ".$data['db_Fname']." ".$data['db_Mname']?></td>
              <td><?php echo $data['db_course']?></td>
              <td><?php echo $data['db_year']?></td>
              <td><?php echo $data['db_section']?></td>
              <td><?php echo $data['db_status']?></td>
              <td><?php echo $data['db_sem']?></td>
              <td><?php echo $data['db_syear']?></td>
              <td><a href='details.php?studNo=<?php echo $data['db_studNo']?>' class='btn btn-dark'>DETAILS</a></td>
            </tr>
        <?php
